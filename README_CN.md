...
真实事故（见 [`HANDOFF.md`](HANDOFF.md) Known Gotchas）逐条从"踩过的坑"
转化为回归测试或审计闸的累积。
...
转化模型（*The Knowledge-Creating Company*, 1995）外化进 `HANDOFF.md`。
...
插件的 skill/命令**强制命名空间化**（Claude Code 官方策略）：子命令变为
`/gitx-release:gitx-sop`、`/gitx-release:gitx-init`（不是扁平的
`/gitx-sop`）。更新：`/plugin marketplace update GitX`。
...
`HANDOFF.md` 里每条 Known Gotcha（37 条）都映射到一个回归测试或审计闸——
缺陷不会静默复发。
...
| SECI / 隐性知识 | `HANDOFF.md` 设计 | Nonaka & Takeuchi, *The Knowledge-Creating Company*, 1995 |
...
| [`HANDOFF.md`](HANDOFF.md) | 开发日志 + 37 条 Known Gotcha（每条 → 一道 guard）|
