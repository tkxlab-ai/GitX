...
> 当前版本号见 [`VERSION`](./VERSION) / [`Release/CHANGELOG.md`](./Release/CHANGELOG.md)，本节不写死版本号（历史教训：硬编码版本描述会 doc-rot）。

支持任何 `skills/<name>/SKILL.md` 布局的项目发版（`tests/run_all.sh` + 双源 `scripts/`），跨四个 CLI 安装面：Claude Code · Codex · OpenCode · Gemini。已在多个 skill 项目（gitx-release 自身 + mac-release 等 sibling）生产验证。子命令 `/gitx-init`（生成 `.gitx/` 政策包）与 `/gitx-sop`（生成 GitHub 公开镜像发布 runbook）随主命令一同分发。

通用化到非-skill 源码包（Swift / Rust / Go / Python）的进展见 [ROADMAP.md](./ROADMAP.md)。

本仓库**用自己的 `scripts/gitx-release.sh` 自举发版**（它本身就是 `skills/gitx-release/SKILL.md` 布局的 skill 项目）；安装到下游项目后对那些项目同样生效。
...
| 项目初始化 | `/gitx-init` | `scripts/gitx-init.sh` | 在目标项目生成 `.gitx/` 政策包 + `RELEASE_GUIDELINE.md`（auto-detect skill/mac/both/empty）|
| GitHub 发版 SOP | `/gitx-sop` | `scripts/gitx-sop.sh` | 在目标项目生成 `.gitx/GITHUB_RELEASE_SOP.md`（GitHub 公开镜像发布 runbook，占位符渲染，**只生成不执行 git/gh**）|
