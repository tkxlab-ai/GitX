...
[![Tests](https://img.shields.io/badge/tests-94%20suites%20%2F%200%20fail-brightgreen.svg)](tests/run_all.sh)
...
of 30+ real incidents (see [`HANDOFF.md`](HANDOFF.md) Known Gotchas) each
...
behavior; tacit operational knowledge is externalized into `HANDOFF.md`
following *Nonaka & Takeuchi*'s SECI model of knowledge conversion (*The
...
Plugin skills/commands are **namespaced** (Claude Code policy): the
subcommands become `/gitx-release:gitx-sop` and `/gitx-release:gitx-init`
(not the flat `/gitx-sop`). Updates: `/plugin marketplace update GitX`.
...
Every Known Gotcha in `HANDOFF.md` (37 entries) maps to a regression test or
an audit gate — defects do not recur silently.
...
| SECI / tacit knowledge | `HANDOFF.md` design | Nonaka & Takeuchi, *The Knowledge-Creating Company*, 1995 |
...
| [`HANDOFF.md`](HANDOFF.md) | Dev log + 37 Known Gotchas (each → a guard) |
