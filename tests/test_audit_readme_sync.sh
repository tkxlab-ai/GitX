...
# docs-pipeline.sh uses PROJECT_ROOT="${PROJECT_ROOT:-$(pwd)}"; the test-runner
# exports PROJECT_ROOT=<real-repo-root>, so each behavioral subshell must
# override it to the fixture tmpdir so --init/--check operate on the fixture,
# not the live repo (root cause of "Can't open README.md" under run_all.sh).
mk(){ mkdir -p "$1/tests"; : > "$1/tests/test_suite_structure.sh"; : > "$1/tests/test_a.sh"; echo v1.0.0 > "$1/VERSION"; }
D1="$(mktemp -d)"; mk "$D1"; ( cd "$D1" && PROJECT_ROOT="$D1" bash "$GR" --init --force >/dev/null 2>&1 )
( cd "$D1" && PROJECT_ROOT="$D1" bash "$GR" --check >/dev/null 2>&1 ) && ok "in-sync adopted README → --check 0 (§0g would PASS)" || fail "in-sync false-drift"
D2="$(mktemp -d)"; mk "$D2"; ( cd "$D2" && PROJECT_ROOT="$D2" bash "$GR" --init --force >/dev/null 2>&1 )
( cd "$D2" && perl -0pi -e 's{(<!-- gitx:managed:suite-count -->\n).*?(\n<!-- /gitx:managed:suite-count -->)}{${1}999STALE${2}}s' README.md )
( cd "$D2" && PROJECT_ROOT="$D2" bash "$GR" --check >/dev/null 2>&1; [ $? -eq 1 ] ) && ok "stale managed region → --check 1 (§0g would FAIL)" || fail "managed drift not caught"
D2b="$(mktemp -d)"; mk "$D2b"; ( cd "$D2b" && PROJECT_ROOT="$D2b" bash "$GR" --init --force >/dev/null 2>&1 && printf '\nunmanaged prose edit\n' >> README.md )
( cd "$D2b" && PROJECT_ROOT="$D2b" bash "$GR" --check >/dev/null 2>&1 ) && ok "unmanaged-prose edit → --check 0 (§0g PASS, projen contract)" || fail "§0g false-FAIL on unmanaged prose"
D2c="$(mktemp -d)"; mk "$D2c"; ( cd "$D2c" && PROJECT_ROOT="$D2c" bash "$GR" --init --force >/dev/null 2>&1 )
...
( cd "$D2c" && PROJECT_ROOT="$D2c" bash "$GR" --check >/dev/null 2>&1; [ $? -eq 1 ] ) && ok "stale whats-new → --check 1 (§0g would FAIL, generic for dependent skills)" || fail "whats-new drift not caught"
D2d="$(mktemp -d)"; mk "$D2d"; ( cd "$D2d" && PROJECT_ROOT="$D2d" bash "$GR" --init --force >/dev/null 2>&1 )
( cd "$D2d" && perl -0pi -e 's{(<!-- gitx:managed:command-surface -->\n).*?(\n<!-- /gitx:managed:command-surface -->)}{${1}STALE-CS${2}}s' README.md )
( cd "$D2d" && PROJECT_ROOT="$D2d" bash "$GR" --check >/dev/null 2>&1; [ $? -eq 1 ] ) && ok "stale command-surface → --check 1 (§0g would FAIL)" || fail "command-surface drift not caught"
D3="$(mktemp -d)"; mk "$D3"; echo '# no markers' > "$D3/README.md"
( cd "$D3" && PROJECT_ROOT="$D3" bash "$GR" --check >/dev/null 2>&1 ) && ok "no-marker README → --check 0 (§0g SKIPs, generic-safe)" || fail "no-marker false-fail"
