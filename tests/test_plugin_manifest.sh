...
# 10: no stale per-repo "marketplace add tkxlab-ai/GitX" in shipped
# manifests / public docs / this test header (CHANGELOG history is the
# project record and is exempt). Central form is tkxlab-ai/marketplace.
_stale=0
while IFS= read -r _f; do
    # this test legitimately contains the banned literal as the grep
    # pattern itself (enforcement) — exempt it, like test_rebrand exempts
    # itself for the legacy name it bans.
    case "$_f" in */Release/*|*/CHANGELOG.md|*/.git/*|*tests/test_plugin_manifest.sh) continue ;; esac
    _stale=1; fail "stale per-repo marketplace add in $_f"
done < <(grep -rl 'marketplace add tkxlab-ai/GitX' \
            --include='*.md' --include='*.json' --include='*.sh' --include='*.txt' \
            "$ROOT" 2>/dev/null | grep -vE '/Release/|/\.git/|CHANGELOG\.md|tests/test_plugin_manifest\.sh')
