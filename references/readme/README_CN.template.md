...
> 🛠 **实时构建指标** — 版本 **v1.11.0** · 发版日期 **2026-05-16** · 由 **Claude（Opus/Sonnet）· Codex · Gemini** 跨数百会话打造 · 自首个原型（v0.9.4，2026-04-22）起累计 AI token 估算：**≈ 3 亿+ 输入/输出 + ≈ 30 亿+ 缓存** · 跨度：**26 天 ~60 个发布**（2026-04-22 → 2026-05-18）
<!-- /gitx:managed:build-metrics -->
...
| 版本 | 日期 | 要点 |
|---|---|---|
| v1.11.0 | 2026-05-16 | 独立的双语文档流水线 + 硬 fail-closed `docs-audit`；流水线现在跑与 GitHub CI 同款的 shellcheck 闸（§0j） |
| v1.10.1 | 2026-05-16 | 机器派生的更新摘要 + 命令面 managed 区块；graphify-out/CLAUDE.md 五-facet 私有态加固；五轮双引擎闭环 |
| v1.10.0 | 2026-05-16 | projen 范式 README 代笔器（`gitx-readme`，确定性，`--check`）+ §0g/§0h/§0i 审计闸 |
| v1.9.8 | 2026-05-16 | README 数值漂移根治 + `§0f` 文档数值漂移守护 |
| v1.9.7 | 2026-05-16 | gitx-sop 凭证门系统性修复；五-facet 纵深防御 |
<!-- /gitx:managed:whats-new -->
...
| 触发 | 入口 | 功能 |
|---|---|---|
| `/gitx-release` | `scripts/gitx-release.sh` | 一键发版：自动递增 patch 版本 → 同步 SKILL.md + CHANGELOG → 跑完整 release 闸。`--version vX.Y.Z` 指定显式版本。不自动 `git push`。 |
| `release <version>` | `scripts/release.sh` | 显式版本发版：测试 → 打包（.skill + tarball）→ 平摊文档 + install.sh → sanity 扫描 → CHANGELOG → 深度审计。不自动 `git push`。 |
| `audit <version>` | `scripts/release-audit.sh` | 对已有 `Release/<ver>/` 重跑 40+ 项静态审查（断网也能跑）。 |
| `scan <dir>` | `scripts/release-sanitize.sh` | 扫描指定目录的 6 类敏感信息（凭证 / 绝对用户路径 / 真实邮箱 / 公网 IP / MAC / SSH-GPG 指纹）。 |
| `/gitx-init` *（亦：`init guideline` · 项目初始化）* | `scripts/gitx-init.sh` | 生成 `.gitx/` 政策包 + `RELEASE_GUIDELINE.md`；auto-detect skill / mac / both / empty。 |
| `/gitx-sop` *（亦：推 GitHub）* | `scripts/gitx-sop.sh` | 渲染 `.gitx/GITHUB_RELEASE_SOP.md` —— GitHub 公开发布 runbook（只生成，不碰 git/gh）。 |

`/gitx-release` / `/gitx-init` / `/gitx-sop` 是斜杠命令；`release` / `audit` / `scan` 是自然语言触发（英文措辞见 [English README](README.md)）。`git push` / `gh release` 始终需用户显式确认（政策 §10.10）。

### 安装方式 vs 调用面

**A. install.sh（扁平，全 CLI）** — 即上方 SKILL.md 命令集：斜杠 `/gitx-release` · `/gitx-init` · `/gitx-sop`，加自然语言 `release <ver>` · `audit <ver>` · `scan <dir>`。Codex 选择器：`$gitx-release` · `$gitx-init` · `$gitx-sop` · `$git-release-pipeline`*（`$gitx-release` 的弃用别名；仅 codex-commands.txt，不是斜杠命令）*。OpenCode/Gemini 按意图触发（"gitx release"）。

**B. 插件安装（marketplace）** — `/plugin marketplace add tkxlab-ai/marketplace` 再 `/plugin install gitx@tkx-skills`。插件（`gitx`）把命令命名为 `/gitx:<cmd>`（冒号命名空间是插件独有设计，依官方文档）。**安装后完整命令列表：**

| 命令 | 功能 | 脚本入口 |
|---|---|---|
| `/gitx:release` | 一键发版：fail-closed 闸门 + 密钥扫描 + 可复现 tarball + CycloneDX SBOM + Deep Audit。`git push`/`gh release` 仍需用户显式确认（政策 §10.10）。 | `scripts/gitx-release.sh` |
| `/gitx:audit` | Deep Audit —— 对发版产物运行只读审计闸门（`§0a–§0j`，三态）。 | `scripts/release-audit.sh` |
| `/gitx:scan` | 密钥 / 敏感串扫描与脱敏 —— 发版前清理闸门。 | `scripts/release-sanitize.sh` |
| `/gitx:init` | 生成 `.gitx/` 政策包 + `RELEASE_GUIDELINE.md`（自动识别 skill / mac / both / empty）。 | `scripts/gitx-init.sh` |
| `/gitx:sop` | 渲染 `.gitx/GITHUB_RELEASE_SOP.md` —— GitHub 公开发布 runbook（只生成，不碰 git/gh）。 | `scripts/gitx-sop.sh` |
<!-- /gitx:managed:command-surface -->
