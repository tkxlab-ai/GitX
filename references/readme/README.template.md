...
|---|---|---|
| v1.11.0 | 2026-05-16 | Independent bilingual documentation pipeline + hard fail-closed `docs-audit`; pipeline now runs the same shellcheck gate GitHub CI enforces (§0j) |
| v1.10.1 | 2026-05-16 | Machine-derived What's-New + command-surface managed regions; graphify-out/CLAUDE.md five-facet private-state hardening; 5-round dual-engine closure |
| v1.10.0 | 2026-05-16 | projen-style README ghostwriter (`gitx-readme`, deterministic, `--check`) + §0g/§0h/§0i audit gates |
| v1.9.8 | 2026-05-16 | README numeric-rot root fix + `§0f` doc-numeric-rot guard |
...
Triggered by the slash command **or** by natural-language intent (release / ship / publish / audit / version-bump). Chinese trigger phrases are documented in the [中文 README](README_CN.md).

Mirrors the authoritative command contract in `SKILL.md`:

| Trigger | Entry | What it does |
|---|---|---|
| `/gitx-release` | `scripts/gitx-release.sh` | One-key release: auto-increments the patch version → syncs SKILL.md + CHANGELOG → runs the full release gate. `--version vX.Y.Z` for an explicit version. No auto `git push`. |
| `release <version>` | `scripts/release.sh` | Explicit-version release: tests → package (.skill + tarball) → flatten docs + install.sh → sanity scan → CHANGELOG → Deep Audit. No auto `git push`. |
| `audit <version>` | `scripts/release-audit.sh` | Re-run the 40+ static checks over an existing `Release/<ver>/` (works offline). |
| `scan <dir>` | `scripts/release-sanitize.sh` | Scan a directory for the 6 sensitive classes (credentials / abs user paths / real email / public IP / MAC / SSH-GPG fingerprint). |
| `/gitx-init` *(also "init guideline")* | `scripts/gitx-init.sh` | Scaffold `.gitx/` policy pack + `RELEASE_GUIDELINE.md`; auto-detect skill / mac / both / empty. |
| `/gitx-sop` *(also "publish to GitHub")* | `scripts/gitx-sop.sh` | Render `.gitx/GITHUB_RELEASE_SOP.md` — the GitHub-publish runbook (generate-only, no git/gh). |

`/gitx-release` / `/gitx-init` / `/gitx-sop` are slash commands; `release` / `audit` / `scan` are natural-language triggers (Chinese equivalents in the [中文 README](README_CN.md)). `git push` / `gh release` always require explicit user confirmation (policy §10.10).

### Install vs. invocation surface

**A. install.sh (flat, all CLIs)** — the SKILL.md command set above: slash `/gitx-release` · `/gitx-init` · `/gitx-sop` plus natural-language `release <ver>` · `audit <ver>` · `scan <dir>`. Codex selectors: `$gitx-release` · `$gitx-init` · `$gitx-sop` · `$git-release-pipeline` *(deprecated alias of `$gitx-release`; codex-commands.txt only — not a slash command)*. OpenCode/Gemini trigger by intent ("gitx release").

**B. Plugin install (marketplace)** — `/plugin marketplace add tkxlab-ai/marketplace` then `/plugin install gitx@tkx-skills`. The plugin (`gitx`) namespaces its commands as `/gitx:<cmd>` (colon namespace is a plugin-only design, per official docs). **Full command list after install:**

| Command | What it does | Script entry |
|---|---|---|
| `/gitx:release` | One-key release: fail-closed gates + secret scan + reproducible tarball + CycloneDX SBOM + Deep Audit. `git push`/`gh release` still require explicit user confirmation (policy §10.10). | `scripts/gitx-release.sh` |
| `/gitx:audit` | Deep Audit — read-only audit gate over a release artifact (`§0a–§0j`, three-state). | `scripts/release-audit.sh` |
| `/gitx:scan` | Secret / sensitive-string scan & sanitize — the pre-release cleanup gate. | `scripts/release-sanitize.sh` |
| `/gitx:init` | Scaffold `.gitx/` policy pack + `RELEASE_GUIDELINE.md` (auto-detects skill / mac / both / empty). | `scripts/gitx-init.sh` |
| `/gitx:sop` | Render `.gitx/GITHUB_RELEASE_SOP.md` — the GitHub-publish runbook (generate-only, no git/gh). | `scripts/gitx-sop.sh` |
<!-- /gitx:managed:command-surface -->
